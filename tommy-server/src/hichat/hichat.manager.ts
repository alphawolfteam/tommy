...
export default class HichatManager {
    ...
    public static async fetchAuthData() {
        ...
        if (res.data && res.data.status === 'success') {
            ...
            console.log(`user id: ${HichatManager.userId}`);
            await HichatManager.setRedis('hi-user-id', JSON.stringify(userId));
            HichatManager.authToken = authToken;
            console.log(`auth token: ${HichatManager.authToken}`);
            await HichatManager.setRedis('hi-auth-token', JSON.stringify(authToken));
        } else {
    ...
    private static async removeMemberFromGroupFactory(roomName: string, member: string): Promise<void> {
        ...
        if (member !== config.chat.loginUser) {
            await axios
                .post(`${config.chat.chatUrl}/${config.chat.chatGroupUrl}.kick`, { roomName, username: member }, { headers: { ...authHeaders } })
                .catch((error) => {
                    logger(error);
                });
        }
    ...
    public static async setRoomAnnouncement(roomName: string, announcement: string) {
        ...
        if (result && result.data) {
            ...
            await axios
                .post(
                    `${config.chat.chatUrl}/${config.chat.chatGroupUrl}.setAnnouncement`,
    ...
    private static async addMemberToGroupFactory(roomName: string, member: string) {
        ...
        const result = await axios.post(
            `${config.chat.chatUrl}/${config.chat.chatGroupUrl}.invite`,
    ...
    public static async sendMessageToGroup(userT: string, text: string) {
        ...
        await axios
            .post(
                `${config.chat.chatUrl}/${config.chat.chatMessageUrl}.postMessage`,
                { channel: `#${normalizedGroupName}`, text },
                { headers: { ...authHeaders } },
            )
            .then((res) => res.data)
            .catch((error) => {
    ...
    private static getAllowedGroupTitleFromText(title: string): string {
        if (HichatManager.cache && HichatManager.cache[title]) return HichatManager.cache[title];
        ...
        const allowedTitle = title.replace(regexExppresion, (str) =>
            str
                .split('')
                .map(() => '.')
                .join(''),
    ...
    public static async isGroupExists(userT: string): Promise<any> {
        const builtTommyGroupName = HichatManager.buildGroupName(userT);
        const normalizedGroupName = HichatManager.getAllowedGroupTitleFromText(builtTommyGroupName);
        return HichatManager.getGroupInfo(normalizedGroupName)
            .then(() => ({ exists: true, groupName: normalizedGroupName }))
            .catch(() => ({ exists: false }));
    }
